...
let private setWindowsAppTheme (light: bool) =
    ...
    with _ ->
        failwith
            "No permission to modify AppsUseLightTheme value. Electron will hang. Change Windows app theme to Light manually before starting Electron."


let init () =

    if ``process``.platform = Win32
       && main.nativeTheme.shouldUseDarkColors then
        main
            .app
