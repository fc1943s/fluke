...
let runTool cmd (args: string) workingDir =
    ...
    if result.ExitCode <> 0 then failwithf "'%s %s' failed" cmd args
...
Target.create "Clean" (fun _ -> Shell.cleanDirs [ distDir ])
...
Target.create "Build" (fun _ -> runTool yarnTool "run webpack -p" path)
...
Target.create "FastRun" (fun _ -> run ())
